...
### 3.4MinorGC和FullGC触发条件

MinorGC是发生在新生代的，这里主要使用的是**复制算法**。

MajorGC/FullGC是发生在老年代的，老年代的主要使用**标记清除或者标记整理算法**。
...
此方法的调用是建议JVM进行Full GC，虽然只是建议而非一定，但很多情况下它会触发Full GC，从而增加Full GC的频率，即增加了间歇性停顿的次数。强烈建议能不使用此方法就别使用。

**CMS GC异常**

MinorGC时，survivor空间放不下，对象只能放入老年代，而老年代也放不下造成。

**堆内存分配很大的对象**

**所谓大对象，是指需要大量连续内存空间的java对象**，例如很长的数组，此种对象会直接进入老年代，而**老年代虽然有很大的剩余空间，但是无法找到足够大的连续空间来分配给当前对象**，此种情况会触发JVM进行FullGC。
